...
with tab1:
    ...
    file_sheet2 = col2.file_uploader("Upload Sheet 2 (B,D,F,G)", type=["xlsx"])
    ...
    if file_amu and file_sheet2:
        try:
            ...
            # Case-insensitive cleaning for matching
    ...
    if st.session_state['master_df'] is not None:
        ...
        for i, current_month in enumerate(month_list):
            ...
            if not month_df.empty:
                ...
                it_move = col1.selectbox("Move Item:", month_df['Item'], key=f"s_{i}")
                if col2.button("Postpone ➡️", key=f"b_{i}"):
                    idx = df[df['Item'] == it_move].index
                    # Increment the TargetDate by one month
                    df.loc[idx, 'TargetDate'] = df.loc[idx, 'TargetDate'] + pd.DateOffset(months=1)
                    st.session_state['master_df'] = df
                    st.rerun()
            else:
                st.write("No items forecast.")
            st.divider()
